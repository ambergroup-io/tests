...
use crate::util::{
    deploy_builtin_contract, deploy_contract, hex_to_byte32, hex_to_bytes, hex_try_to_byte32,
    mock_cell_with_outpoint, mock_dep, mock_input, mock_output, mock_script, u32_to_uint32,
    u64_to_uint64,
};
use ckb_testtool::{
...
impl<'a> TemplateParser<'a> {
        ...
        for item in self.data.contracts.clone() {
            match &item.mode[..] {
                "deployed" => {
                    ...
                    self.deps.push(cell_dep.clone());
                    self.contracts.insert(item.name, script.code_hash());
                }
                _ => {
    ...
    fn parse_script_hash(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        for item in self.data.block_list.clone() {
            let h = RawHeader::new_builder()
                ...
                .dao(hex_try_to_byte32(&item.dao[..])?)
                .build();
    ...
    fn parse_cell(
        ...
        for it in cells {
            ...
            let lock_script = self
                .parse_script(it.lock_script)
                .expect("error lock script");
            let type_script = self
                .parse_script(it.type_script)
                .expect("error type script");

            let data_str = VARIABLE_REG
                .replace_all(&it.data[..], |caps: &Captures| {
            ...
            // parse data of cell
            let data =
                hex_to_bytes(&data_str[..]).expect(format!("error data:{}", data_str).as_str());
            ...
            match mode {
                ...
                "output" => {
                    self.outputs
                        .push(mock_output(it.capacity, lock_script.unwrap(), type_script))
                }
                _ => panic!("unknow mode"),
            }
            ...
            if it.block_id > 0 {
                ...
                self.context
                    .link_cell_with_block(outpoint.clone(), block_hash, tid);
            }
        }
    ...
    fn parse_script(&mut self, script_info: JsonScript) -> Result<Option<Script>, Box<dyn Error>> {
        ...
        let hash_str = VARIABLE_REG
            .replace_all(&script_info.code_hash[..], |caps: &Captures| {
                let script_name = caps.get(1).map(|m| m.as_str()).unwrap();
            ...
            .to_string();
        let code_hash =
            hex_to_byte32(&hash_str[..]).expect(format!("error code_hash:{}", hash_str).as_str());

        let args_str = VARIABLE_REG
            .replace_all(&script_info.args[..], |caps: &Captures| {
                let script_name = caps.get(1).map(|m| m.as_str()).unwrap();
                let hash = match self.script_hash.get(script_name) {
                    Some(h) => h.clone(),
                    None => match self.contracts.get(script_name) {
        ...
        if script_info.name != "" {
            self.script_hash
                .insert(script_info.name, script.calc_script_hash());
        }
        Ok(Some(script))
    }
    ...
    fn parse_witnesses(
        ...
        for it in self.data.witnesses.clone() {
            let args_str = VARIABLE_REG
                ...
                .to_string();
            let data = hex_to_bytes(&args_str[..])
                .expect(format!("error witnesses:{}", args_str).as_str());
            datas.push(data.clone());
        }
